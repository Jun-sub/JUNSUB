function [output,paras] = BSL_func_EISmodel_V_half_Dist_integrated(f_vector,factors,soc,T,type_acf,type_dist)
...
% para = [dUdc,Rp,Ds,F,sp,Cdl,Rct,C_film,Rf,mu_drt,sig_drt,mu_ddt,sig_ddt]
    if para(11) > 0.001
        ...
        elseif type_dist == 2 % Integrated
            ...
            % test
            upper_func_p2d = @(t)lognpdf(t,para(12),para(13)).*zloc_func_p2d(t,para,type_dist); upper_p2d = integral(upper_func_p2d,0.001,100);
            down_func_p2d = @(t)lognpdf(t,para(12),para(13)); down_p2d = integral(down_func_p2d,0.001,100);
            % test

            beta_drt = (upper_drt./down_drt)^-1/para(4);
            beta_ddt = upper_ddt./down_ddt/para(4); 
            beta_p2d = (upper_p2d./down_p2d)^-1/para(4);
            beta = beta_drt + beta_ddt - beta_p2d;
        end
    else
...
function zloc = zloc_func_p2d(~,para,~) % LGES V2 2024 05
% para = [dUdc,Rp,Ds,F,sp,Cdl,Rct,C_film,Rf,mu_drt,sig_drt,mu_ddt,sig_ddt]; 
