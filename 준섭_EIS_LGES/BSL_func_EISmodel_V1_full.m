function [output,paras] = BSL_func_EISmodel_V1_full(f_vector,factors,soc,T,type_acf)
...
if type_acf ~=4
    error('change function to V1_3E model') % 풀 셀이 아니면 작동 안되게
end
...
elseif type_acf ==4 % full cell
    output = [R_itsc_n+R_itsc_p+(1/A_coat)*real(a_imp.')+(1/A_coat)*real(c_imp.')+2.*real(s_imp.') (1/A_coat)*imag(a_imp.')+(1/A_coat)*imag(c_imp.')+2.*imag(s_imp.')];
    paras =[R_itsc_n, i0a, Cdla, Dsa, aa, R_itsc_p, i0c, Cdlc, Dsc, ac, kappa, Di0]';
else
    error('select anode (1), cathode (2), 3E cell (3), full cell (4)')
end
