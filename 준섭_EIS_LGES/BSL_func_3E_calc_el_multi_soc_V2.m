...
elseif type_acf == 4 
    ...
    factors_integ_ini(1:5,2*length(multi_soc_range)+1) = [1;1;1;1;0.237]; % for Kel; Del; Av_n; Av_p; epsla
    factors_integ_ini(1:6,2*length(multi_soc_range)+2) = [1;1;1;1;1;1]; %for a_n1;a_n2;a_n3;a_n4;a_n5;a_p

    % temporary for test
    % load("C:\Users\admin\Documents\GitHub\JunSub\준섭_EIS_LGES\10월 미팅\Av_integ_V2\241208_Test_P2D_Kel,Del fix\3E simul P2D + soc integrated + DRT + DDT integ workspace.mat")
    ...
    lb(5,2*length(multi_soc_range)+1) = 0.1; %for Epsla 
    ub(5,2*length(multi_soc_range)+1) = 0.4;
    ...
    factors_integ_ini_dist(1:length(factors_ini)/2,1:2*length(multi_soc_range)+2) = factors_integ_hat;
