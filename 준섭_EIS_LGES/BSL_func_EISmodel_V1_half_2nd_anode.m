function [output,paras] = BSL_func_EISmodel_V1_half_2nd_anode(f_vector,factors,soc,T,type_acf,type_anode) %normal V1_half
...
sa=1i*omegagen(j)*epsla*La^2/Dieffa;          %[refer to list of symbols]        
sc=1i*omegagen(j)*epslc*Lc^2/Dieffc;          %[refer to list of symbols]                    
ss=1i*omegagen(j)*epsls*Lsep^2/Dieffs;        %[refer to list of symbols]        

%s2a/s2c - corresponds to the anode/cathode particle
spa=1i*omegagen(j);      %   s2a                 %[refer to list of symbols]        
spc=1i*omegagen(j);      %   s2c                 %[refer to list of symbols]        
...
%New Beta based on Meyers Case 2
betaa = 1/F*(spa*C_filma + (spa*Cdla + 1/Rcta)/(1+(spc*Cdla + 1/Rcta)*Rfa)); % eqn [54] in 2007 paper % local impedance [m2/ohm] times (1/F)
betac = 1/F*(spc*C_filmc + (spc*Cdlc + 1/Rctc)/(1+(spc*Cdlc + 1/Rctc)*Rfc)); % eqn [54] in 2007 paper % local impedance [m2/ohm] times (1/F)
...
lambda1a = 1/2*(sa + B1a + B2a + sqrt(sa^2 + 2*B1a*sa - 2*sa*B2a + B1a^2 + 2* B1a*B2a + B2a^2));    % [Eqn [5]in paper]
lambda1c = 1/2*(sc +   + B2c + sqrt(sc^2 + 2*B1c*sc - 2*sc*B2c + B1c^2 + 2* B1c*B2c + B2c^2));    % [Eqn [5]in paper]
...
%Additional calcualtion of Z_RC & Z_w |Ds = Inf
%Z_RC
b_hf_c = Lc^2*ac*(1/sigmaeffc + 1/kappaeffc)*(spc*C_filmc + (spc*Cdlc + 1/Rctc)/(1 + ((spc*Cdlc + 1/Rctc))*Rfc)); % eqn [56] in 2007 paper
b_hf_a = La^2*aa*(1/sigmaeffa + 1/kappaeffa)*(spa*C_filma + (spa*Cdla + 1/Rcta)/(1 + ((spa*Cdla + 1/Rcta))*Rfa)); % eqn [56] in 2007 paper

Z_RCc(j) = (Lc/(sigmaeffc + kappaeffc))*(1 + 1/((sqrt(b_hf_c)*tanh(sqrt(b_hf_c))))*(sigmaeffc/kappaeffc + kappaeffc/sigmaeffc) + 2/(sqrt(b_hf_c)*sinh(sqrt(b_hf_c)))); %eqn[55] in 2007 paper
Z_RCa(j) = (La/(sigmaeffa + kappaeffa))*(1 + 1/((sqrt(b_hf_a)*tanh(sqrt(b_hf_a))))*(sigmaeffa/kappaeffa + kappaeffa/sigmaeffa) + 2/(sqrt(b_hf_a)*sinh(sqrt(b_hf_a)))); %eqn[55] in 2007 paper

%Z_w |Ds = Inf

phi1x1rcc = - Lc^3*ac*F*betac/sigmaeffc^2*((sc-lambda1c)/B1c/lambda1c*(GAMMALAMBDAC1)+(sc-lambda2c)/B1c/lambda2c*(GAMMALAMBDAC2))+C_7_c+C_8_c; % eqn[31]
phi1x1rca = - La^3*aa*F*betaa/sigmaeffa^2*((sa-lambda1a)*C_1_a/B1a/lambda1a+(sa-lambda2a)*C_3_a/B1a/lambda2a) + C_8_a; % eqn[30]

Z_wec(j) = -phi1x1rcc;
Z_wea(j) = -(phi2_sep_xs_0-phi1x1rca);
end
