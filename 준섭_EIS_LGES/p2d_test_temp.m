...
% soc range 
    ...
    save_path = 'G:\공유 드라이브\Battery Software Lab\Projects\LGES 2023\발표 및 공유자료\10월 미팅자료\흑연 2종 결과 데이터\천연_100%'; %파라미터 결과 저장 폴더 경로 지정 
    save_check = 0; % 피규어 및 파라미터 데이터 저장 유무 선택, 0이면 결과 저장 x, 1이면 결과 저장 O
    % 주의: 동일한 폴더에 동일한 type_acf, type_dist 사용시 기존 파일 삭제 후 저장됨

% EIS data path
    path_folder = 'G:\공유 드라이브\BSL-Data\LGES\Modeling data_2차 음극 2종';%불러올 데이터 폴더 경로 지정
    path_file = '%s_soc%d.txt'; % %s 및 soc%d 부분은 고정, 그 외의 파일 이름만 수동으로 지정 ex) 파일 이름: soc20_cathode_LGES 라면, soc%d_cathode_LGES로 지정
    %주의: 데이터 형식 변경되면 적용 불가, 파일 이름에 % 특수기호 삭제 후 진행

% Fitting configuration
    type_weight = 1; % 0 for absolute error, 1 for relative error
    type_acf = 1; % 1 for anode
    type_anode = 1; %1 for blending, 2 for natural graphite 100%
    type_dist = 2; % 0 for DRT, 1 for DDT, 2 for integrated
    num_iter = 50; %P2D 최적화 과정 최대 반복 횟수
...
for i = 1:length(soc_vec)
    ...
    if type_anode == 1
        cell_type = '인조_천연 Blending음극_#1';
    elseif type_anode == 2
        cell_type = '천역흑연100%_음극_#2';
    end 
    SOC = soc_vec(i);
...
%% Load and Pre-processing Data
    ...
    % for naming
    if type_anode == 1
        cell_type = 'Art&Natural Blending';
    elseif type_anode == 2
        cell_type = 'Natural 100';
    end 
