function [f_data, z_integ_data, z_model, z_model_dist, paras_integ ,paras_integ_dist] = BSL_func_3E_calc_el_multi_soc_V_final2(path_folder, path_file, multi_soc_range, cell_type, anode_type)
...
%% 3E_Simul
    ...
    for i = 1:2*length(multi_soc_range)
        ...
        % Data without trim
        if i <= length(multi_soc_range)
               dataa_trim(:, 3*i-2:3*i) = data(:, 3*i-2:3*i);

        elseif i > length(multi_soc_range)
               datac_trim(:, 3*i-2:3*i) = data(:, 3*i-2:3*i);
               
        end 

        %data rearrangement (integrate freq data)
        if i <= length(multi_soc_range)
               z_integ_dataa(:,2*i-1:2*i) = dataa_trim(:,3*i-1:3*i);
               % anode [real(i) imag(1) ... real(i) imag(i)]
        elseif i > length(multi_soc_range)
    ...
    [z_model, paras_integ] = BSL_func_soc_integrated_model_V_final2_3E(f_data,factors_integ_hat,multi_soc_range,T,type_acf,cell_type);
