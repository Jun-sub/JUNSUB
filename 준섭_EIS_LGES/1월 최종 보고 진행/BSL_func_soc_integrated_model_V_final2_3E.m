function [output,paras] = BSL_func_soc_integrated_model_V_final2_3E(f_vector,factors,multi_soc_range,T,type_acf,cell_type)
...
%% Inductance
   L1 = factors(1,2*length(multi_soc_range)+2)*1e-11;
   L2 = factors(2,2*length(multi_soc_range)+2)*1e-11;
%% Kinetics Parameters
    ...
    % epsla = factors(5,i);        epslc = factors(5,length(multi_soc_range)+i);          % {modified} void fraction LGES 2024 02
...
% Parameter Expressions
    ...
    y_shift = (soc <= 0.6)*factors(5,2*length(multi_soc_range)+1) + (soc > 0.6)*factors(6,2*length(multi_soc_range)+1);

    % dUdc is applied directly in soc scale in v3
    dUdcc = -((1/ctc)*(Uc_function_v3(soc+dx) - Uc_function_v3(soc-dx))/(2*dx));   % {modified}
    dUdca = ((1/cta)*(Ua_function_v3(soc+dx) - Ua_function_v3(soc-dx))/(2*dx))-y_shift;
...
parasc(:,i) = [R_itscc(i), i0c, Cdlc, Dsc, ac, kappa, Di0]';
    end
...
if type_acf == 4 % 3E_Simul
        
            for i = 1:length(multi_soc_range)
                output(:,2*i-1) = R_itsca(i)+(1/A_coat)*real(a_imp(:,i)); %Anode real
                output(:,2*(i+length(multi_soc_range))-1) = R_itscc(i)+(1/A_coat)*real(c_imp(:,i)); %Cathode real

                output(:,2*i) = (1/A_coat)*imag(a_imp(:,i)); %Anode imag
                output(:,2*(i+length(multi_soc_range))) = (1/A_coat)*imag(c_imp(:,i)); %Cathode imag

                paras(:,i) = [parasa(:,i);parasc(:,i)];
            end
