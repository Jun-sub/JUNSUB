...
% EIS data path
    ...
    save_path = 'C:\Users\admin\Documents\GitHub\JunSub\준섭_EIS_LGES\Full_cell_fig';
...
%% Load and Pre-processing Data
    ...
    figure(1)
    plot(z_data(:,1),-z_data(:,2),'ro','MarkerFaceColor','red'); hold off; grid on %트림한 데이터 플랏 full cell
...
%% FITTING
%   Call EIS model
   weighted_model = @(factors,f_data)BSL_func_EISmodel_V1_full(f_data, factors,soc,T,type_acf).*weight_matrix; %모델 예측값에 가중치 곱하여 모델 설정
...
%% Plot Results
[z_model0, paras0] = BSL_func_EISmodel_V1_full(f_data,factors_ini,soc,T,type_acf);
[z_model1, paras1] = BSL_func_EISmodel_V1_full(f_data,factors_hat,soc,T,type_acf);
...
plot(z_model1(:,1),-z_model1(:,2),'o','linewidth',1,'color',cmat_n(2,:)) %full model fit plot
