...
% Fitting configuration
    ...
    num_iter = 30; %P2D optimization max iter
...
if type_acf == 4 
    ...
    for i = 1:length(soc_vec)
        ...
        if soc_integ == 1 && i == 1
           [f_data, z_data_integ_sep, z_model_integ_sep, z_model_dist_integ_sep, paras_integ ,paras_integ_dist] = BSL_func_3E_calc_el_multi_soc_V3(path_folder, path_file, multi_soc_range, cell_type, []);
           ...
           t = tiledlayout(ceil(sqrt(length(multi_soc_range))),ceil(sqrt(length(multi_soc_range))),"TileSpacing","loose","Padding","loose");
            for i = 1:length(multi_soc_range)
                ...
                legend(['z data' ' soc ' num2str(multi_soc_range(i))],['z model' ' soc ' num2str(multi_soc_range(i))])
                title('Anode')
